...
@@ -53,8 +53,12 @@ ALL_CFLAGS = $(CFLAGS) @GLIB_CFLAGS@ @GSL_CFLAGS@ @PLPLOT_CFLAGS@ \
 	@PGPLOT_CFLAGS@ 
...
@@ -71,12 +75,13 @@ SERVER_LIBS =  @XMLRPC_SERVER_LIBS@
 SWIG = @SWIG@
...
@@ -84,6 +89,10 @@ $(TARGETS): setupdata.py $(MYLIBS)
 	python makesetup.py
...
@@ -87,7 +87,7 @@ all: install
 $(TARGETS): setupdata.py $(MYLIBS)
 	...
 	python makesetup.py
-	python setup.py build install --install-lib=.
+	ARCHFLAGS="-arch x86_64" python setup.py build install --install-lib=.
